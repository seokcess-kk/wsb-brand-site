  ...
  return (
    ...
    >
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-24 md:py-32 lg:py-40">
        ...
        {/* KPI grid */}
        <dl className="mt-16 grid gap-px bg-canvas/10 sm:grid-cols-2 lg:grid-cols-4">
          {KPI_KEYS.map((key, i) => (
        ...
        <div className="mt-20">
          ...
          <div className="grid gap-px bg-canvas/10 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
            {partners.map((p, i) => (
              <RevealOnView
                key={p.name}
                delay={0.04 * i}
...
function KpiCell({
  ...
  return (
    <div className="group relative h-full bg-structural p-7 transition-colors hover:bg-structural/95">
      ...
      <p className="mt-5 text-sm leading-relaxed text-canvas/65 max-w-[26ch]">
        {caption}
      </p>
      <span
        aria-hidden
...
function PartnerSlot({ name, tag }: { name: string; tag: string }) {
  return (
    <div className="group flex h-full min-h-[120px] flex-col justify-between bg-structural p-5 transition-colors hover:bg-structural/95">
      <p className="mono-label text-[9px] text-canvas/45">{tag}</p>
      <div className="space-y-2">
        <p className="font-sans text-base font-semibold text-canvas">{name}</p>
        <p className="mono-label text-[9px] text-canvas/30">LOGO PENDING</p>
      </div>
    </div>
  );
}
