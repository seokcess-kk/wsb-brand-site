...
node scripts/screenshot.mjs / out.png                          # 전체 페이지 (기본 BASE: localhost:3002)
node scripts/screenshot.mjs / hero.png desktop "section[...]"  # 특정 selector만
# SHOT_BASE 환경변수로 포트 override
```
...
**1. 공개 사이트 `src/app/[locale]/`** — next-intl 다국어. `routing.ts`의 `localePrefix: "as-needed"`로 한국어는 prefix 없는 `/`, 영문은 `/en`. 각 페이지는 `src/components/sections/*`(콘텐츠 블록)와 `src/components/visual/*`(커스텀 SVG/데이터 시각화: FDA 매칭표, MAT 다이어그램, growth curve 등)를 조합한다. 모션은 `motion` 패키지(ex-Framer Motion)를 `src/components/motion/*` 래퍼로 감싸 사용.
...
- ORM: Drizzle + Neon serverless Postgres. 스키마: `src/db/schema.ts` (테이블: `inquiries`, `news_posts`, `site_settings`, `uploaded_files`). 클라이언트: `src/db/client.ts`.
...
- `src/env.ts`가 모든 env를 Zod로 중앙 파싱. 빈 문자열 = 미설정으로 취급. 누락 허용.
