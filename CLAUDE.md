...
- ORM: Drizzle + Supabase Postgres (`postgres.js` 드라이버, `drizzle-orm/postgres-js`). 스키마: `src/db/schema.ts` (테이블: `inquiries`, `news_posts`). 클라이언트: `src/db/client.ts`.
...
- 모든 쓰기는 `src/app/actions/*`의 **서버 액션** 경유(`inquiry`, `news` 2개). Zod로 입력 검증 후 `revalidatePath`로 어드민/공개 경로 갱신.
- 재사용 쿼리는 `src/lib/*-query.ts`(`inquiries-query`, `news-query`)로 중앙화 — 어드민 대시보드와 목록이 공유.
- 뉴스 등록 시 외부 URL에서 본문·메타데이터를 자동 추출한다(`src/lib/url-metadata.ts`). SSRF 방어는 `src/lib/url-safety.ts`(프라이빗·로컬 IP 차단)에서 담당하므로 외부 URL fetch는 반드시 이 가드를 거칠 것. 기타 공용 헬퍼: `datetime`·`truncate`·`page-metadata`·`mailto`·`inquiry-status`.

### Graceful degradation (중요)

env가 비어 있어도 dev 빌드/렌더가 통과하도록 설계됨. 이 패턴을 깨지 말 것:

- `src/env.ts`가 주요 env(`DATABASE_URL`, `RESEND_*`, `INQUIRY_NOTIFY_TO`, `AUTH_SECRET`, `NEXT_PUBLIC_SITE_URL`)를 Zod로 파싱. 빈 문자열 = 미설정으로 취급. 누락 허용. 단 `ADMIN_EMAIL`/`ADMIN_PASSWORD`(`src/auth.ts`)와 `DIRECT_DATABASE_URL`(`drizzle.config.ts`)은 env.ts를 거치지 않고 `process.env`에서 직접 읽는다. 문의 알림 수신처는 Settings 제거 후 `INQUIRY_NOTIFY_TO` env가 단일 출처(미설정 시 `dasom@woorismartbio.com` 기본값). 환경변수 전체 목록과 Vercel 배포 절차는 `README.md`·`.env.example` 참고.
...
- 러너는 **Playwright 하나**로 통일. `tests/unit/*`은 순수 lib 함수 검증(`datetime`, `inquiry-status`, `mailto`, `truncate`, `url-metadata`, `url-safety`)이라 브라우저가 필요 없고, `tests/e2e/*`은 실제 플로우(문의 워크플로우)를 돈다.
